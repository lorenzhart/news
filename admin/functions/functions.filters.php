 ...
 * 
 * @since 1.4.0
 * @param $admin_data Options array
 * @return array
 */
function of_filter_save_media_upload($admin_data) {
	if($admin_data){
		foreach ($admin_data as $key => $value) {
			if (is_string($value)) {
				$admin_data[$key] = str_replace(
					array(
						site_url('', 'http'),
						site_url('', 'https'),
					),
 ...
 * @return array
 */
function of_filter_load_media_upload($admin_data) {
