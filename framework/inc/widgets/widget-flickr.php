<?php
/**
 * Plugin Name: MyPassion Flickr Widget
...
class flickr extends WP_Widget {
	...
	function flickr() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'flickr-widget', 'description' => __('A widget that show last flickr photo streams', 'flickr-widget') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'flickr-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'flickr-widget', __('MyPassion - Flickr', 'framework'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Flickr Widget', 'Flickr Widget'), 'id' => __('52617155@N08', 'Flickr Witdget'), 'show_num' => '8', 'how' => 'latest');
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e('Flickr id', 'example'); ?></label>
		...
		<p>
            ...
            <br/>
			<label><?php _e('Max is "18"', 'example'); ?></label>
		</p>
		
        <p>
			<label for="<?php echo $this->get_field_id( 'how' ); ?>"><?php _e('What pictures to display', 'framework'); ?></label><br />
